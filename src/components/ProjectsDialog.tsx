...
export default function ProjectsDialog({ onClose }: { onClose: () => void }) {
  ...
  const [teamTrash, setTeamTrash] = useState<Project[] | null>(null);
  ...
  const loadTrash = useCallback(
    async (teamId: string) => {
      ...
      try {
        const token = await getToken();
        if (!token) throw new Error("Not signed in");
        const raw = await listTeamTrash(teamId, "workback", token);
        const docs = Object.values(raw)
          .map((d) => {
            try {
              return migrate(d);
            ...
            {showTrash ? (
              <TeamTrashList
                ...
                onRecover={async (p) => {
                  ...
                  if (!token) return;
                  try {
                    await recoverTeamDoc(scope.teamId, "workback", p, token);
                    setTeamTrash((cur) => cur?.filter((x) => x.id !== p.id) ?? null);
                    setTeamProjects((cur) => (cur ? [p, ...cur] : cur));
                  } catch {
                ...
                onPurge={async (p) => {
                  if (scope.kind !== "team") return;
                  if (!confirm(`Permanently delete “${p.title || "Untitled Workback"}”? This can't be undone.`)) return;
                  const token = await getToken();
                  if (!token) return;
                  try {
                    await purgeTeamDoc(scope.teamId, "workback", p.id, token);
                    setTeamTrash((cur) => cur?.filter((x) => x.id !== p.id) ?? null);
                  } catch {
              ...
              <TeamList
                ...
                onDelete={async (p) => {
                  if (scope.kind !== "team") return;
                  if (!confirm(`Delete “${p.title || "Untitled Workback"}” from this team? Members will lose access; it can be recovered from Trash.`)) return;
                  const token = await getToken();
                  if (token) {
                    try {
...
function TeamTrashList({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-hairline">
      {projects.map((p) => (
        <div key={p.id} className="flex items-center gap-2 border-b border-hairline px-3 py-2.5 last:border-b-0">
          <div className="min-w-0 flex-1">
            <div className="truncate text-[13px] font-semibold">{p.title || "Untitled Workback"}</div>
            <div className="truncate text-[11.5px] text-ink-soft">
